    ...
    public function create($amenidad_id)
{
    ...
    // Añadir la propiedad disponible a cada horario
    $horariosDisponibles = $amenidad->horarios->map(function ($horario) use ($reservados, $fechaSeleccionada) {
        $horario->disponible = true;
        foreach ($reservados as $reservado) {
            if (
                ($horario->start_time >= $reservado->start_time && $horario->start_time < $reservado->end_time) ||
                ...
                // Verificar si la hora actual es mayor que la hora de finalización de la reserva
                if (Carbon::parse($fechaSeleccionada)->isToday() && Carbon::now()->format('H:i') > $reservado->end_time) {
                    continue;
                }
                $horario->disponible = false;
                break;
